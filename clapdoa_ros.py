"""
Clap + DOA detector, publishes /clap_doa_angle
"""
...
# Path to ReSpeaker tuning
sys.path.append('/home/mattllacza/usb_4_mic_array')
...
def init_doa():
    ...
    tuning = Tuning(dev)
    print("DOA angle:", tuning.direction)
...
def compute_features(block):
    ...
    band_en, total_en = block_band_energy(block)
    band_ratio = band_en / (total_en + 1e-12)
...
def passes_decay(c_energy, future):
    if not future:
        return False
    r1 = future[0] / (c_energy + 1e-12)
    rmin = min(future) / (c_energy + 1e-12)
    return (r1 < DECAY_ALPHA) and (rmin < DECAY_BETA)

# AUDIO CALLBACK
def on_audio(indata, frames, time_info, status):
    global last_clap_time, pending_candidate, pending_future_energies, pending_frames_left
    ...
    if pending_candidate is not None:
        ...
        if pending_frames_left <= 0:
            ...
            ok_dec = passes_decay(cand["band_energy"], pending_future_energies)

            if ok_dec and (now - last_clap_time) > COOLDOWN:
                last_clap_time = now
                angle = None
                try:
                    angle = float(tuning.direction)
                except:
                    pass

                ts = time.strftime('%H:%M:%S')
                if angle is not None:
                    print(f"[{ts}] Clap, angle={angle:.1f}")
                    doa_pub.publish(Float32(data=angle))
                else:
                    print(f"[{ts}] Clap, no angle")
...
def main():
    global doa_pub
    rospy.init_node('clap_doa_node')
    doa_pub = rospy.Publisher('/clap_doa_angle', Float32, queue_size=10)

    init_filters()
    init_doa()

    print("Device:", sd.query_devices(DEVICE)['name'])
    print("Clap + DOA running...")
